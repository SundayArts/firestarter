...
contract Crowdsale is Owned {
    ...
    function Crowdsale() public {
        ...
        fundingGoal = @funding_goal@ * 1 ether;
        price = 1 ether / @price@;
        transferableToken = @transferable_token@ * (10 ** uint(tokenReward.decimals()));
        startTime = @start_time@;

        // ここどうしましょう
        sales[i].start = start[i];
        sales[i].end = end[i];
        sales[i].rate = rate[i];

        count++;
        deadline = sales[conut].end;
    }
    ...
    function () public payable isStopped {
        ...
        uint token = amount * (10 ** uint256(tokenReward.decimals())) / currentSwapRate();
    ...
    // Get the rate of the token. Set from week 1 to week 4
    function currentSwapRate() private constant returns(uint) {
        for (uint i = 0; i < count; i++) {
            if (sales[i].start <= now || now <= sales[i].end) {
                return sales[i].rate;
            }
        }
        return price;
    ...
    function checkGoalReached() private afterDeadline isStopped {
        if (isOpened) {
